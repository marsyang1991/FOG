...
def load_data():
    ...
    file.close()
    # data = make_sequential_data2()
    # train_set = np.array(data['train_set'])
    # train_set_y = np.array(data['train_y'])
    # validate_set = np.array(data['validate_set'])
    # validate_set_y = np.array(data['validate_y'])
    # test_set = np.array(data['test_set'])
    # test_set_y = np.array(data['test_y'])
...
if __name__ == "__main__":
    ...
    if True is training:
        ...
        model.fit(x=train_set, y=train_y, batch_size=1, epochs=10,
                  validation_data=(validate_set, validate_y),callbacks=[TensorBoard(log_dir='./temp/log_rnn')])
        model.save('rnn')
    else:
