...
class _BaseIOHMM:
    ...
    def fit(self):
        ...
        for n in range(self.n_iter):
            ...
            for i in range(len(obs)):
                ...
                log_trans_mat = self._compute_log_transmat(
                    ins_seq)  # compute dynamic transition matrix with shape (t, n, n)
        ...
        def hess_func(thetas):
            ...
            for s in range(self.n_components):
                ...
                for p in range(self.n_components):
                    ...
                    hess_item = X.T * dia_matrix((prob_item, 0), shape=(len(prob_item), len(prob_item))) * dia_matrix(
                        (sum_trans_post, 0), shape=(len(sum_trans_post), len(sum_trans_post))) * X
                    hess_array[(s * self.input_dim):((s + 1) * self.input_dim),
                    (p * self.input_dim):((p + 1) * self.input_dim)] = np.array(hess_item)
        ...
        for n in range(n_iter):
            ...
            for s in range(self.n_components):
                ...
                for p in range(self.n_components):
                    ...
                    hess_item = X.T * dia_matrix((prob_item, 0), shape=(len(prob_item), len(prob_item))) * dia_matrix(
                        (sum_trans_post, 0), shape=(len(sum_trans_post), len(sum_trans_post))) * X
                    consize_hess_array[(s * self.input_dim):((s + 1) * self.input_dim),
                    (p * self.input_dim):((p + 1) * self.input_dim)] = np.array(hess_item)
            ...
            for s in range(self.n_components):
                for p in range(self.n_components):
                    ...
                    hess_array[self.input_dim * (s): self.input_dim * (s + 1),
                    self.input_dim * (p): self.input_dim * (p + 1)] = hess_item

            print "hess_array", hess_array
            trans_theta_old = trans_theta
            consize_trans_theta_old = trans_theta
            trans_theta = trans_theta - (np.matrix(np.linalg.pinv(hess_array)) * jac_vec).reshape(self.n_components,
                                                                                                  self.input_dim)
            difference.append(np.max(trans_theta_old - trans_theta))

            try:
                consize_trans_theta = consize_trans_theta_old - np.reshape(
                    np.linalg.pinv(consize_hess_array) * np.matrix(consize_jac_array).flatten().T,
    ...
    def optimize_trans_beta(self, ins_seq, obs_seq, j, n_iter, threshold=1e-6):
        ...
        for n in range(n_iter):
            ...
            try:
                trans_theta = trans_theta - np.reshape(np.linalg.pinv(hess_array) * jac_vec,
                                                       (self.n_components, self.input_dim))
            except Exception as e:
                print 'Failed to Converge!'
                print 'jac_vec', jac_vec
    ...
    def _compute_sufficient_static(self, log_trans_mat, log_frame_prob, log_fwd_lattice, log_bwd_lattice,
                                   log_likelihood):
        ...
        for j in range(self.n_components):
            for i in range(self.n_components):
                trans_posts[0][j][i] = np.exp(
                    log_frame_prob[0, i] + log_start_prob[j, 0] + log_bwd_lattice[0, i] + log_trans_mat[
                        0, j, i] - log_likelihood)

        for t in range(1, T):
            for j in range(self.n_components):
                for i in range(self.n_components):
                    trans_posts[t][j][i] = np.exp(
                        log_frame_prob[t, i] + log_fwd_lattice[t - 1, j] + log_bwd_lattice[t, i] + log_trans_mat[
                            t, j, i] - log_likelihood)
