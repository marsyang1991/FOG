...
class TestIOHMM(unittest.TestCase):
    def setUp(self):
        ...
        X1 = np.floor(3 * Y + 2 + np.random.np.random.normal(0, sd_noise, sample_size))
        X2 = np.floor(7 * Y + 5 + np.random.normal(0, sd_noise, sample_size))
    ...
    def _test_compute_log_trans(self):
        self.hmm.trans_weight_mat[:, :, :] = 1
        print self.hmm.trans_weight_mat
        log_trans_prob = self.hmm._compute_log_transmat(self.hmm.ins[0])

    def _test_log_obs(self):
        obs_seq = self.hmm.obs[0]
        ins_seq = self.hmm.ins[0]
        print self.hmm.obs_weight_mat
        print 'obs', obs_seq
        print 'ins_seq', ins_seq
        print np.exp(ins_seq * np.matrix(self.hmm.obs_weight_mat).T)
        frame_prob = self.hmm._compute_log_obs_prob(ins_seq, obs_seq)
        print frame_prob

    def _test_forward_lattice(self):
        ins_seq = self.hmm.ins[0]
        obs_seq = self.hmm.obs[0]
        trans_mat = self.hmm._compute_log_transmat(ins_seq)  # compute dynamic transition matrix with shape (t, n, n)
        frame_prob = self.hmm._compute_log_obs_prob(ins_seq, obs_seq)
        lpr, fwd_lattice = self.hmm._do_forward_pass(trans_mat, frame_prob)
        print lpr, fwd_lattice
